...
One Python process exposing two surfaces:
  - POST /event  : FastAPI receiver for hook payloads (Claude Code, Codex,
                   OpenCode, Pi). Always returns {} so hooks never block.
  - /            : Gradio dashboard polling the in-memory event buffer,
                   re-rendering once a second.

Run:   python app.py        -> http://localhost:8000
Hook:  POST http://localhost:8000/event
"""
...
async def event(req: Request):
    ...
    # Empty body so hook callers never block on the dashboard receiver.
    return JSONResponse({})
